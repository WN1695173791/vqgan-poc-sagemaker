...
import json
import aws_util
import taming.data.utils as data_utils
...
class SetupCallback(Callback):
    ...
    def on_epoch_end(self, trainer, pl_module):
        if trainer.global_rank == 0:
            ...
            copy_tree(self.logdir, os.environ['SM_CKPT_DIR'])
...
if __name__ == "__main__":
    ...
    print(f"[PRE] ********** os.environ {os.environ}")
    now = datetime.datetime.now().strftime("%Y-%m-%dT%H-%M-%S")
    ### Add for SageMaker
    sm_host = json.loads(os.environ['SM_HOSTS'])
#     sm_host = ['algo-1'] # LOCAL-1

    for i in range(int(os.environ['SM_NUM_GPUS'])):
        gpus = '0' if i == 0 else gpus + ',' + str(i)

    os.environ['MASTER_ADDR'] = sm_host[0] # LOCAL-1

    job_name = json.loads(os.environ['SM_TRAINING_ENV'])['job_name'] # LOCAL-1
#     job_name = 'local-1'
    os.environ['JOB_NAME'] = job_name


    sm_ckpt_dir = os.path.join(os.environ['SM_CHANNEL_TRAINING'], 'vqgan_poc', job_name)
...
#         gpu_mon_thread = aws_util.GPUMon(device_index= local_rank, job_name=job_name)
...
#         logdir = os.path.join("logs", job_name)
    ...
    #         trainer_config['profiler'] = 'advanced'
        ...
        os.environ['NODE_RANK'] = str(
            int(os.environ['CURRENT_HOST'].split('-')[1]) - 1)

        print(f"[AFTER] ********** os.environ {os.environ}")
        ### Finish to add for SageMaker
        ...
        def melk(*args, **kwargs):
            ...
            if trainer.global_rank == 0:
                ...
                ckpt_path = os.path.join(ckptdir, "last_exce.ckpt")
                trainer.save_checkpoint(ckpt_path)
...
#             gpu_mon_thread.kill() 
        if trainer.global_rank == 0:
            ...
            sync_local_checkpoints_to_s3(
                os.environ['SM_CKPT_DIR'],
                os.path.join(os.environ['OUTPUT_S3'], os.environ['JOB_NAME'],"final_ckpt"))
